...
def dump_topology_to_string(
    ...
    if s == TopAbs_COMPOUND or s == TopAbs_SOLID:
        ...
        topology_shapes.append(shape)
        topology.append(level)
    ...
    while it.More() and level < 4:  # LEVEL MAX
        shp = it.Value()
        it.Next()
        #if shp.ShapeType == TopAbs_COMPOUND or it.Value == shp.ShapeType:
        dump_topology_to_string(shp, level + 1, buffer)
        
    return topology, topology_shapes
